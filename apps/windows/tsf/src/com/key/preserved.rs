//! 「翻译选中文字」「Ctrl+Space 切换中英」与「朗读译文」三个快捷键登记成 TSF **保留键**（preserved key）。
...
/// 读「朗读译文」的组合（`[shortcut] speak_translation`）；读不到 / 解析失败用缺省。
pub(crate) fn load_speak_combo() -> KeyCombo {
    let Some(path) = qingjian_platform::dirs::config_path() else {
        return KeyCombo::SPEAK_DEFAULT;
    };
    match Config::load(&path) {
        Ok(config) => config.shortcut.speak_keys(),
        Err(error) => {
            log(&format!("读配置取朗读快捷键失败，用缺省: {error}"));
            KeyCombo::SPEAK_DEFAULT
        }
    }
}

/// 登记「朗读译文」保留键。
pub(crate) fn register_speak(keystroke: &ITfKeystrokeMgr, tid: u32, combo: KeyCombo) -> Result<()> {
    let key = preserved_key(combo);
    let description: Vec<u16> = "朗读译文".encode_utf16().collect();
    unsafe { keystroke.PreserveKey(tid, &GUID_SPEAK, &key, &description) }
}

pub(crate) fn unregister_speak(keystroke: &ITfKeystrokeMgr, combo: KeyCombo) {
    let key = preserved_key(combo);
    let _ = unsafe { keystroke.UnpreserveKey(&GUID_SPEAK, &key) };
}
