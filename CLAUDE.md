...
本地构建也可走 `task`（go-task，`winget install Task.Task`）：`task check` 一键 fmt + clippy + 全量测试（非 macOS 自动排除 IMK 壳），`task dll-register` / `task server` / `task tts-smoke` 跑 Windows 输入法，全部任务见 `Taskfile.yaml`。
